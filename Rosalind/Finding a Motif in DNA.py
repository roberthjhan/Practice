...
Solution works when I copy the Rosalind data to sample.txt, rosalind_subs.txt
is for some reason not working...
"""

def findMotiff(s, t):
    ret = ""
    for i in range(len(s) - len(t) + 1): # +1 to check last possible slot
        # print(len(s[i: i + len(t)]), len(t))
        if s[i: i + len(t)] == t:
            ret += str(i + 1) + " "
    return ret
...
print(t in s) # Something strange, not returning True when it should be...
print(findMotiff(s, t))
