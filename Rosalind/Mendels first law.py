...
'''
Probability of recessive pair mating
n/(k+m+n) * (n-1)/(k+m+n-1)
1.00 recessive

Probability of heterozygous pair mating
m/(k+m+n) * (m-1)/(k+m+n-1)
0.25 recessive 

Probability of heterozygous - recessive mating
m/(k+m+n) * (n)/(k+m+n-1) + n/(k+m+n) * (m)/(k+m+n-1)
0.50 recessive

Dom phenotype will be 1- P_rec_phenotype
'''

def domProb(k,m,n):
    tot = k+m+n
    pRR = n/(tot) * (n-1)/(tot-1)
    pHetHet = m/(tot) * (m-1)/(tot-1) * 0.25
    pHetR = (m/(tot) * (n)/(tot-1) + n/(tot) * (m)/(tot-1)) * 0.5
    pR_phenotype = pRR + pHetHet + pHetR
    return 1 - pR_phenotype

filename = "rosalind_iprb.txt"
data = open(filename, "r").read()
k, m, n = data.split(" ")
print(domProb(int(k), int(m), int(n)))